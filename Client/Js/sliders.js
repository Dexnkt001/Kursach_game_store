...
if(-id+5<=mass_len){
  ...
  for(let i=-id; i < -id+5-temp;i++){
    mass_place[index].childNodes[1].attributes[0].nodeValue = mass_pict[i].link;
        mass_place[index].childNodes[3].innerHTML = mass_pict[i].name;
        mass_place[index].childNodes[7].innerHTML = mass_pict[i].prize + ' &#8381;';
